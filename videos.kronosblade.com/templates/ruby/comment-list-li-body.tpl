...
<span class="pm-comment-action" id="users-{$smarty.foreach.comment_foreach.iteration}">
	<div class="btn-group pull-right">
		{if $comment_data.user_id > 0 && $comment_data.user_id != $s_user_id && $can_manage_comments && $comment_data.power != $smarty.const.U_ADMIN}
			{if $comment_data.user_is_banned}
				<button class="unban-{$comment_data.user_id} btn btn-mini active" type="button" id="unban-{$comment_data.id}" rel="tooltip" title="{$lang.user_account_remove_ban}"><i class="icon-ban-circle icon-white opac7"></i></button>
			{else}
				<button class="ban-{$comment_data.user_id} btn btn-mini" type="button" id="ban-{$comment_data.id}" rel="tooltip" title="{$lang.user_account_add_ban}"><i class="icon-ban-circle icon-white opac7"></i></button>
			{/if}
		{/if}
		<button class="btn btn-mini {if $comment_data.user_likes_this}active{/if}" type="button" {if $comment_data.user_id != $s_user_id}id="comment-like-{$comment_data.id}"{/if} rel="tooltip" title="{$lang._like}"><i class="icon-thumbs-up icon-white opac7"></i>
		...
		<button class="btn btn-mini {if $comment_data.user_dislikes_this}active{/if}" type="button" {if $comment_data.user_id != $s_user_id}id="comment-dislike-{$comment_data.id}"{/if} rel="tooltip" title="{$lang._dislike}"><i class="icon-thumbs-down icon-white opac7"></i>
		...
		<button class="btn btn-mini {if $comment_data.user_flagged_this}active{/if}" type="button" id="comment-flag-{$comment_data.id}" rel="tooltip" title="{$lang.report_form5}"><i class="icon-flag icon-white opac7"></i></button>
	{if $can_manage_comments}
	<button class="btn btn-mini btn-warning" onclick="onpage_delete_comment('{$comment_data.id}', '{$comment_data.uniq_id}', '#comment-{$smarty.foreach.comment_foreach.iteration}'); return false;" rel="tooltip" title="Delete comment"><i class="icon-remove icon-white opac7"></i></button>
	{/if}
	</div>
</span>
{/if}