...
function animateToPosition(animationOptions){running=true;ul.animate(animCss=="left"?{left:-(calculatedTo*liSize)}:{top:-(calculatedTo*liSize)},$.extend({duration:options.speed,easing:options.easing},animationOptions));}});};$.fn.jCarouselLite.options={btnPrev:null,btnNext:null,btnGo:null,mouseWheel:false,auto:null,speed:200,easing:null,vertical:false,circular:true,visible:3,start:0,scroll:1,beforeStart:null,afterEnd:null};})(jQuery);jQuery(document).ready(function($){var width=$(window).width();if(width>=768){init_jcarousel(true);}else{init_jcarousel(false);}
$(window).resize(function(){var width=$(window).width();if(width>=768){init_jcarousel(true);}else{init_jcarousel(false);}});function init_jcarousel(vertical){$(".lite-carousel-play div[data-carousel=lite]").each(function(){var config={btnNext:".lite-carousel-play .next",btnPrev:".lite-carousel-play .prev",vertical:vertical,circular:false};if($(this).data('visible')){config.visible=$(this).data('visible');}else{config.visible=3;}
$(this).attr('style','');$(this).find('ul').attr('style','');$(this).find('li').attr('style','');if(vertical==false){config.visible=3;var number=$(this).data('number');if(number<=3){$(this).parent().find('.carousel-md').removeClass('hidden').addClass('hidden');}else{$(this).parent().find('.carousel-md').removeClass('hidden');}}else{var number=$(this).data('number');if(number<=config.visible){$(this).parent().find('.carousel-md').removeClass('hidden').addClass('hidden');}else{$(this).parent().find('.carousel-md').removeClass('hidden');}}
var jcarousel=$(this);$(this).jCarouselLite(config);return false;});}});