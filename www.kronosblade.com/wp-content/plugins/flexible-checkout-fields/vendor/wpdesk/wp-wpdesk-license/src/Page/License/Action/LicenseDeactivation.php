 ...
 */
class LicenseDeactivation implements Action {
	...
	public function execute( array $plugin ) {
		...
		if ( $activate_results['deactivated'] === true ) {
			...
			add_settings_error( 'wc_am_deactivate_text',
				...
				__( 'Plugin subscription deactivated. ',
					'wpdesk-helper' ) . "{$activate_results['activations_remaining']}.",
				'updated' );
		...
		if ( ! $deactivated && isset( $activate_results['code'] ) ) {
			...
			switch ( $activate_results['code'] ) {
				case '100':
					add_settings_error( 'api_email_text',
						'api_email_error',
					...
					break;
				case '101':
					add_settings_error( 'api_key_text',
						'api_key_error',
						"{$activate_results['error']}. {$activate_results['additional info']}",
						'error' );
					$options[ $plugin['api_manager']->api_key ]          = '';
				...
				case '102':
					...
					break;
				case '103':
					add_settings_error( 'api_key_exceeded_text',
						'api_key_exceeded_error',
						"{$activate_results['error']}. {$activate_results['additional info']}",
						'error' );
					$options[ $plugin['api_manager']->api_key ]          = '';
					$options[ $plugin['api_manager']->activation_email ] = '';
					...
					break;
				case '104':
					add_settings_error( 'api_key_not_activated_text',
						'api_key_not_activated_error',
						"{$activate_results['error']}. {$activate_results['additional info']}",
						'error' );
					$options[ $plugin['api_manager']->api_key ]          = '';
					$options[ $plugin['api_manager']->activation_email ] = '';
					...
					break;
				case '105':
					add_settings_error( 'api_key_invalid_text',
					...
					break;
				case '106':
					add_settings_error( 'sub_not_active_text',
						'sub_not_active_error',
						"{$activate_results['error']}. {$activate_results['additional info']}",
						'error' );
					$options[ $plugin['api_manager']->api_key ]          = '';
					$options[ $plugin['api_manager']->activation_email ] = '';
